...
namespace ImGui
{
	struct DockContext
	{
		...
		void splits()
		{
			...
			ImU32 color = GetColorU32(ImGuiCol_Border);
		...
		bool tabbar(Dock& dock, bool close_button)
		{
			...
			if (BeginChild(tmp, size, true))
			{
				...
				while (dock_tab)
				{
					...
					draw_list->PathBezierCurveTo(pos + ImVec2(size.x + 5, 0), pos + ImVec2(size.x + 10, size.y), pos + ImVec2(size.x + 15, size.y), 10);
					draw_list->PathFillConvex(hovered ? color_hovered : (dock_tab->active ? color_active : color));*/

					draw_list->AddText(pos + ImVec2(0.0, 1.0), text_color, dock_tab->label, text_end);
